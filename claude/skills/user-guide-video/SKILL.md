...
├── preview.html    # generated by this skill's render.py — never hand-edit
└── clips/
    ├── 01-<step>.mp4
    └── 02-<step>.mp4
```
...
**6. Generate `preview.html`** with the render CLI that ships in this
skill's directory (needs [uv](https://docs.astral.sh/uv/) on the PATH —
it's a single-file Python script with inline dependencies):

```bash
uv run <this-skill's-directory>/render.py --input docs/user-guides/<slug>/doc.yaml
```

It writes `preview.html` next to the doc.yaml (video paths are relative to
the doc, so the page must stay beside `clips/`), and warns on stderr if a
referenced clip is missing. doc.yaml is the source of truth — never edit
the generated HTML by hand; fix the yaml and re-render. The page styling
is a house convention: it lives in one clearly marked block at the top of
`render.py`, ready to be swapped for another team's brand.
